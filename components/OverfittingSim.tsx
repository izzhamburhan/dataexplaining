...
// Validation data (new points the model hasn't seen)
...
const OverfittingSim: React.FC<Props> = ({ adjustment, currentStep = 0, onInteract, onNext, nextLabel, isTourActive, onTourClose }) => {
  ...
  const isIntro = currentStep === 0; // Underfitting Phase
  const isTestingPhase = currentStep === 1; // Overfitting / Validation Phase
  ...
  useEffect(() => { 
    // If moving to testing phase, reset complexity to something moderate
    if (isTestingPhase) {
      setComplexity(5);
    } else {
      setComplexity(1);
    ...
    <div className="bg-white p-12 border border-black/5 shadow-[0_40px_100px_rgba(0,0,0,0.03)] w-full max-w-none flex flex-col items-center relative select-none transition-all duration-700">
      ...
      <div className="relative w-full h-[360px] bg-[#FDFCFB] border border-black/5 overflow-hidden mb-12 shadow-[inset_0_2px_10px_rgba(0,0,0,0.02)]">
        <svg className="absolute inset-0 w-full h-full" viewBox="0 0 100 100" preserveAspectRatio="none">
          ...
          {/* True Underlying Pattern (Phase 1 Only - Educational) */}
          {isIntro && (
            <polyline 
              points={truePath} 
              fill="none" 
          ...
          <polyline 
            ...
            stroke="currentColor" 
            strokeWidth="1.2" 
            className={`transition-all duration-300 ${analysis.color}`} 
          />
        </svg>

        {/* Training Points (Black Diamonds) */}
        {TRAIN_POINTS.map((p, i) => (
          <div 
            key={`train-${i}`} 
            className="absolute w-3 h-3 bg-[#121212] rotate-45 border border-white/50 shadow-sm z-10" 
            style={{ 
              left: `${p.x * 10}%`, 
              top: `${p.y / 2.5}%`, 
        ...
        {/* Validation Points (Red Diamonds - Phase 2 Only) */}
        {isTestingPhase && VALIDATION_POINTS.map((p, i) => (
          <div 
            key={`val-${i}`} 
            className="absolute w-3 h-3 bg-rose-500 rotate-45 border border-white/50 shadow-sm z-10 animate-in zoom-in duration-500" 
            style={{ 
              left: `${p.x * 10}%`, 
              top: `${p.y / 2.5}%`, 
              transform: 'translate(-50%, -50%) rotate(45deg)' 
        ...
        {isIntro && (
          <div className="absolute top-4 left-4 bg-white/80 backdrop-blur-sm border border-black/5 p-4">
            <span className="font-mono text-[9px] font-bold text-[#CCC] uppercase tracking-widest block mb-1">Ground Truth Visible</span>
            <p className="text-[10px] font-serif italic text-slate-500">The dashed line represents the hidden mathematical reality.</p>
          </div>
        )}

        {isTestingPhase && (
          <div className="absolute top-4 right-4 bg-white/80 backdrop-blur-sm border border-rose-100 p-4">
            <span className="font-mono text-[9px] font-bold text-rose-600 uppercase tracking-widest block mb-1">Validation Trial</span>
            <p className="text-[10px] font-serif italic text-slate-500">Red points are unseen data. Can your model predict them?</p>
          </div>
        )}
      </div>
      ...
      <div className="w-full grid grid-cols-1 md:grid-cols-2 gap-12 items-start mb-8">
        <div className="space-y-8">
          ...
          <div className="flex justify-between text-[8px] font-mono font-bold text-[#CCC] uppercase tracking-widest">
            <span>Rigid (Underfit)</span>
            <span>Wobbly (Overfit)</span>
          </div>
        </div>
      ...
      <button 
        ...
        className={`w-full bg-[#121212] hover:bg-[#2A4D69] text-white py-6 font-bold uppercase tracking-[0.3em] text-sm transition-all shadow-xl flex items-center justify-center group ${hasActuallyInteracted ? 'opacity-100' : 'opacity-20 pointer-events-none'}`}
      >
