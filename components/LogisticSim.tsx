...
const LogisticSim: React.FC<Props> = ({ adjustment, currentStep = 0, onInteract, onNext, nextLabel, isTourActive, onTourClose }) => {
  ...
  useEffect(() => {
    // Reset interaction on step change
  ...
  const handleSvgClick = (e: React.MouseEvent<SVGSVGElement>) => {
    ...
    // In intro, we just probe. In engine active, we can add points.
    if (isIntro) {
    ...
    <div className="bg-white p-12 border border-black/5 shadow-[0_40px_100px_rgba(0,0,0,0.03)] w-full max-w-4xl flex flex-col items-center select-none relative transition-all duration-700">
      ...
      <div className="relative w-full h-[320px] bg-[#FDFCFB] border border-black/5 overflow-hidden mb-12 shadow-[inset_0_2px_10px_rgba(0,0,0,0.02)]">
        ...
        >
          ...
          {/* Threshold Line (Phase 2 Only) */}
          {isEngineActive && (
            <line 
                x1="-12" y1={1 - threshold} 
                x2="12" y2={1 - threshold} 
                stroke="#E11D48" 
                strokeWidth="0.01" 
                strokeDasharray="0.1" 
          ...
          {points.map((p, i) => {
            ...
            return (
              <g key={i} transform={`translate(${p.x}, ${1 - p.y})`}>
                <circle 
                    r="0.25" 
                    fill={isCorrect ? "#121212" : "#E11D48"} 
                    className={`transition-all duration-500 ${p.isUserAdded ? 'animate-pulse' : 'opacity-80'}`} 
                />
                {isEngineActive && (
                    <line 
                        x1="0" y1="0" 
          ...
          {/* Probe Label */}
          {lastProbe && (
            <g transform={`translate(${lastProbe.x}, 0.6)`}>
                <text 
                    className="font-mono text-[0.4px] fill-slate-400" 
                    textAnchor="middle"
                >
                    {isIntro ? `Value: ${lastProbe.pred === 1 ? 'YES' : 'NO'}` : `P(y=1): ${lastProbe.prob.toFixed(2)}`}
                </text>
            </g>
          )}
        </svg>
        ...
        {isIntro && (
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none bg-white/5 backdrop-blur-[1px]">
             <span className="font-mono text-[9px] font-bold text-[#CCC] uppercase tracking-[0.5em]">Phase 1: Binary Observations</span>
          </div>
        )}
      </div>
        ...
        <div className="space-y-6">
          <div>
            ...
            <input 
                ...
                onChange={(e) => { setThreshold(parseFloat(e.target.value)); audioService.play('click'); markInteraction(); }} 
                className="w-full h-px bg-black/10 rounded-full appearance-none cursor-pointer accent-[#E11D48]" 
            />
          </div>
          <div>
            <div className="flex justify-between mb-4">
            ...
            <input 
                type="range" min="-8" max="8" step="0.2" value={bias} 
                onChange={(e) => { setBias(parseFloat(e.target.value)); audioService.play('click'); markInteraction(); }} 
